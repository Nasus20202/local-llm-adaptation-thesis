...
5. **Current step:** review the proposed `development-pilot-public-evaluator-custody-amendment-v1`. If approved, implement the repository-relative evaluator binding and model-facing exclusion checks before preparing and reviewing the 24 development-only evaluator bundles. The amendment does not change scenario bindings, Kubernetes v1.36.4 evidence, evaluator hierarchy, metrics, or final-test custody.
